#include <random>
#include <iomanip>
#include "my_cypress512.hpp"

#define SEED 1

void basic_test() {
    const std::array<uint64_t, 8> masterKey{0x1245A2A12331A4A3,
                                            0xB2CCAA34C2BB7723,
                                            0x1245A2A12331A4A3,
                                            0xB2CCAB34C2BB7723,
                                            0x1245A2A12331A4A3,
                                            ...
                                            0x1245A2A12331A4A3,
                                            0xB2CCAB34C2BB7723};
    const auto auxKey = genAuxiliaryKey(masterKey);
    const auto roundKeys = genRoundKeys(auxKey, masterKey);
    for (auto el : auxKey) {
        std::cout << el << ',';
    }
    std::cout << "\nroundKeys:\n";
    for (auto rkey : roundKeys) {
        for (auto el : rkey) {
            std::cout << el << ',';
        }
        std::cout << '\n';
    ...
    const std::array<uint64_t, 8> plainText{
        1, 2, 3, 0xB2CCAB34C2BB7723, 0x1245A2A12331A4A3, 0xB2CCAA34C2BB7723, 0x1245A2A12331A4A3, 0xB2CCAB34C2BB7723};
    auto cipherText = encryptBlock(plainText, roundKeys);
    std::cout << "\ncipherText:\n";
    ...
    auto decryptedText = decryptBlock(cipherText, roundKeys);
    std::cout << "\ndecr text:\n";
    for (auto el : decryptedText) {
        std::cout << el << ',';
    }
    std::cout << '\n';
}
...
void manualTest1() {
    ...
    const auto auxKey = genAuxiliaryKey(masterKey);
    const auto roundKeys = genRoundKeys(auxKey, masterKey);
    auto cipherText = encryptData(data, roundKeys);

    std::cout << "  plain data:\n    ";
    for (auto el : data) {
        std::cout << std::setfill('0') << std::setw(16) << el;
    }
    std::cout << '\n';

    std::cout << std::hex;
    std::cout << "  cipher data:\n    ";
    for (auto el : cipherText) {
        std::cout << std::setfill('0') << std::setw(16) << el;
    }
    std::cout << '\n';

    auto decryptedText = decryptData(cipherText, roundKeys);
...
void manualTest2() {
    ...
    const std::array<uint64_t, 8> masterKey{1, 2, 3, 5, 4, 6, 8, 7};
    const auto auxKey = genAuxiliaryKey(masterKey);
    const auto roundKeys = genRoundKeys(auxKey, masterKey);
    auto cipherText = encryptData(data, roundKeys);
...
void automaticRandomTest() {
    std::cout << __FUNCTION__ << ':' << '\n';

    auto populateIterable = [](auto& iterable) {
        // static std::random_device rd;  //Will be used to obtain a seed for the random number engine
        static std::mt19937 gen(SEED);  // Standard mersenne_twister_engine seeded with rd()
        static std::uniform_int_distribution<uint64_t> uni_dist{};

        std::for_each(std::begin(iterable), std::end(iterable), [](uint64_t& word) { word = uni_dist(gen); });
    };

    auto assertDecryptionValidity = [](const auto& data, const auto& roundKeys) {
        auto cipherText = encryptData(data, roundKeys);
        auto decryptedText = decryptData(cipherText, roundKeys);
        return std::mismatch(std::begin(data), std::end(data), std::begin(decryptedText)).first == data.end() ? true
                                                                                                              : false;
    };

    for (int i = 0; i < 100; ++i) {
        // 8 uint64_ts make 1 block
        // 2 blocks make 1 KB
        // 2000 blocks make 1 MB
        std::vector<uint64_t> data(8 * 2000);
        populateIterable(data);

        std::array<uint64_t, 8> masterKey;
        populateIterable(masterKey);

        const auto auxKey = genAuxiliaryKey(masterKey);
        const auto roundKeys = genRoundKeys(auxKey, masterKey);

        if (!assertDecryptionValidity(data, roundKeys)) {
            std::cout << "  failed on seed = " << SEED << ", iter = " << i << '\n';
        } else {
            std::cout << "  iter " << i << " success\n";
        }
    }
...
int main() {
    ...
    // random masterKey, random plainText
    automaticRandomTest();
}