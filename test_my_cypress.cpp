...
void basic_test() {
    const std::array<WordType, 8> masterKey{0x1245A,
                                            0xB2CCA,
                                            0x1245A2,
                                            0xB2CCAB3,
                                            0x124,
    ...
    const std::array<WordType, 8> plainText{
        1, 2, 3, 4, 5, 6, 7, 8};
...
void manualTest1() {
    ...
    std::cout << "  plain data:\n    ";
    for (auto el : data) {
        std::cout << std::setfill('0') << std::setw(HEX_DIGITS_IN_WORD) << el;
    }
    std::cout << '\n';
...
void manualTest2() {
    ...
    const std::array<WordType, 8> masterKey{1, 2, 3, 5, 4, 6, 8, 7};
    const auto auxKey = genAuxiliaryKey(masterKey);
    const auto roundKeys = genRoundKeys(auxKey, masterKey);
    auto cipherText = encryptData(data, roundKeys);
...
void automaticRandomTest() {
    ...
    auto populateIterable = [](auto& iterable) {
        ...
        static std::uniform_int_distribution<WordType> uni_dist{};

        std::for_each(std::begin(iterable), std::end(iterable), [](WordType& word) { word = uni_dist(gen); });
    };
    ...
    for (int i = 0; i < 100; ++i) {
        // 8 WordTypes make 1 block
        // 2 blocks make 1 KB
        // 2000 blocks make 1 MB
        std::vector<WordType> data(8 * 2000);
        populateIterable(data);

        std::array<WordType, 8> masterKey;
